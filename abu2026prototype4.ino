...
// ==========================================
// ตัวแปรสำหรับเลือกฝั่งและตรวจสอบการกดปุ่ม (เพิ่มใหม่)
// ==========================================
int teamMode = 0;  // 0 = ฝั่งน้ำเงิน (Blue), 1 = ฝั่งแดง (Red)
...
void loop() {
  // ==========================================================
  // 1. ระบบตรวจสอบการกดปุ่ม (Short Press / Long Press + Servo Indicator)
  // ==========================================================
  bool currentBtnSeq = digitalRead(btnSeq_Pin);

  // --- ตรวจจับตอนเริ่มกดปุ่มลง (Rising Edge) ---
  if (lastBtnSeqState == LOW && currentBtnSeq == HIGH) {
    btnPressTime = millis();
    isBtnPressed = true;
  }
  // --- ตรวจจับตอนปล่อยปุ่ม (Falling Edge) ---
  else if (lastBtnSeqState == HIGH && currentBtnSeq == LOW && isBtnPressed) {
    isBtnPressed = false;
    unsigned long pressDuration = millis() - btnPressTime;

    // ทำงานเฉพาะตอนที่ยังไม่เริ่ม Sequence
    if (isSystemRunning && sequenceState == 0) {
      if (pressDuration >= 50 && pressDuration < 500) {
        // กดสั้น (50 - 500 ms): สลับฝั่ง น้ำเงิน/แดง
        teamMode = (teamMode == 0) ? 1 : 0;

        if (teamMode == 0) {
          setServoAngle(40);  // ฝั่งน้ำเงิน: เซอร์โวอยู่ที่ 40 องศา (ตำแหน่งปกติ)
          Serial.println("\n[MODE] >>> ฝั่งน้ำเงิน (BLUE) <<<");
        } else {
          setServoAngle(70);  // ฝั่งแดง: เซอร์โวยกขึ้นไปที่ 70 องศา (บอกสถานะให้รู้)
          Serial.println("\n[MODE] >>> ฝั่งแดง (RED) <<<");
        }
      } else if (pressDuration >= 1000) {
        // กดยาว (> 1000 ms): เริ่มต้นกระบวนการ
        Serial.println("\n[SEQ] === เริ่มต้นกระบวนการ (LONG PRESS) ===");

        // รีเซ็ตเซอร์โวกลับมาที่ท่าเตรียมพร้อม (40 องศา) เสมอก่อนพุ่งออกไป
        setServoAngle(40);
        delay(200);
...
void runSequence() {
  ...
  switch (sequenceState) {
    ...
    case 7:  // รอจนกว่าจะหมุนเสร็จ
      if (!isAutoTurning) {
        if (teamMode == 1) {
          // ถ้าเป็นฝั่งแดง (Red) ให้ไปสเต็ปสไลด์ซ้าย
          Serial.println("[SEQ] 7.1 ฝั่งแดง: เตรียมสไลด์ซ้าย 30 cm");
          sequenceState = 71;
        } else {
          // ถ้าเป็นฝั่งน้ำเงิน (Blue) ข้ามไปรอสัญญาณ Pin 3 เลย
          Serial.println("[SEQ] 8. ฝั่งน้ำเงิน: รอสัญญาณจาก Pin 3...");
          sequenceState = 8;
        }
      }
      break;
    ...
    // ----------------------------------------------------
    // ลำดับแทรกพิเศษ (เฉพาะฝั่งแดง)
    // ----------------------------------------------------
    case 71:  // สไลด์ซ้าย 30 cm
      autoStartX = global_X;
      autoStartY = global_Y;
      autoTargetDistance = 30.0;
      isAutoMoving = true;
      resumePID();

      // สั่งงาน Mecanum ให้สไลด์ซ้ายโดยใช้แกน Y (Vy)
      // *หากลองรันแล้วหุ่นสไลด์ขวา ให้แก้ 1200.0 เป็น -1200.0
      driveKinematics(0.0, 1200.0, 0.0);

      Serial.println("[SEQ] 7.1 เริ่มสไลด์ซ้าย 30 cm...");
      sequenceState = 72;
      break;

    case 72:  // รอหุ่นยนต์สไลด์ซ้ายจนถึงเป้าหมาย
      if (!isAutoMoving) {
        Serial.println("[SEQ] 8. รอสัญญาณจาก Pin 3...");
        sequenceState = 8;
      }
      break;
      // ----------------------------------------------------
